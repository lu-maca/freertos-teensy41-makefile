...
static int obdh_init_start_tasks(int address)
{
  // initialize drivers
  drivers::uart_init();
  csp_init();
...
static void obdh_init_main_task(void* arg)
{
  auto addr = static_cast<int*>(arg);
  obdh_init_start_tasks(*addr);

  vTaskDelete(nullptr);
}

int obdh_init(int addr)
{
  xTaskCreate(obdh_init_main_task, "obdhInit", 512, nullptr, 9, nullptr);
  return CSP_ERR_NONE;
}